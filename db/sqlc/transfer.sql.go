...
const deleteTranfers = `-- name: DeleteTranfers :exec
DELETE FROM tranfers WHERE id = $1
`

func (q *Queries) DeleteTranfers(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, deleteTranfers, id)
	return err
}
...
const updateTranfers = `-- name: UpdateTranfers :one
UPDATE tranfers SET amount = $2 WHERE id = $1 RETURNING id, from_account_id, to_account_id, amount, created_at
`

type UpdateTranfersParams struct {
	ID     int64 `json:"id"`
	Amount int64 `json:"amount"`
}
